...
fn benchmark(file: &str) {
    ...
    println!("Number of correct scores: {} ({:.2}%)", n_correct, n_correct as f32 / times.len() as f32 * 100.0);
    println!("Average time taken: {}μs", times.iter().sum::<u128>() as f32 / times.len() as f32);
    println!("Average nodes searched: {}", nodes_searched.iter().sum::<usize>() as f32 / nodes_searched.len() as f32);
    println!("Average nodes searched per second: {} Kpos/s", nodes_searched.iter().sum::<usize>() as f32 / times.iter().sum::<u128>() as f32 * 1_000.0);
}

fn main() {
    benchmark("benchmarks/Test_L3_R1.txt");  // End game - Easy
    println!("==============================");
    //benchmark("benchmarks/Test_L2_R1.txt");  // Mid game - Easy
}
